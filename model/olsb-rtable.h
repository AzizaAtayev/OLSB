...
#ifndef OLSB_RTABLE_H
#define OLSB_RTABLE_H
...
public:
   ...
   * \param qsize the queue size of the next hop
  ...
  RoutingTableEntry (Ptr<NetDevice> dev = 0, Ipv4Address dst = Ipv4Address (), uint32_t seqNo = 0,
                     Ipv4InterfaceAddress iface = Ipv4InterfaceAddress (), uint32_t hops = 0, uint32_t qsize = 0, Ipv4Address nextHop = Ipv4Address (),
...
private:
  ...
  uint32_t m_hops;
  /// Queue Size of the next hop unit
  uint32_t m_qsize;
  /**
...
/**
 * \ingroup olsb
 * \brief The Routing table used by OLSB protocol
 */
class RoutingTable
{
public:
